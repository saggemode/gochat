...
func main() {
	...
	if err := database.AutoMigrate(ctx, db, "call", log); err != nil {
	...
	callpb.RegisterCallServiceServer(grpcServer, server.New(repo, redisClient, log))
	...
	go func() {
		log.Info("call service started", zap.String("addr", addr))
	...
	<-quit
	log.Info("shutting down call service...")
	grpcServer.GracefulStop()
	log.Info("call service stopped")
}
